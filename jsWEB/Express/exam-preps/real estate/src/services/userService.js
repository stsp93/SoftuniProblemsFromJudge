...
async function register(user) {
    const { username,name, password, rePassword } = user;
    ...
    try {
        ...
        return await signJWT(newUser.username,newUser.name, newUser._id)
...
async function login(user) {
    ...
    try {
        // Sign and return JWT
        return await signJWT(existingUser.username,existingUser.name, existingUser._id)
    } catch(err) {
        console.log(err);
    }
}
