function lockedProfile() {
    ...
    async function loadProfiles() {
        try {
            ...
            const markup = Object.entries(data).map(([_, profile], i) => `<div class="profile">
            ...
            mainEl.insertAdjacentHTML('beforeend', markup.join(''));
            markup.forEach((_, i) => document.querySelector(`#user${i}HiddenFields`).style.display = 'none');

            mainEl.addEventListener('click', function (e) {
                if (e.target.tagName !== 'BUTTON') return;
                const [locked, unlocked, _] = e.target.parentElement.querySelectorAll('input');
                if (locked.checked) return;

                const id = +unlocked.name[4];

                document.querySelector(`#user${id}HiddenFields`).style.display = document.querySelector(`#user${id}HiddenFields`).style.display === '' ? 'none' : '';
